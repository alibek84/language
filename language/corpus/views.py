        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                if not Word.objects.filter(name = current_word).exists():			 
                    ...
                    derivation = form.cleaned_data['derivation']
                    tense = form.cleaned_data['tense']
                    prc = form.cleaned_data['prc']
                    gna = form.cleaned_data['gna']
                    gpr = form.cleaned_data['gpr']
                    ger = form.cleaned_data['ger']
                    num = form.cleaned_data['num']
                    tag = tag + " " + verb + " " + derivation + " " + tense + " " + mood + " " + prc + " " + gna + " " + gpr + " " + ger + " " + pronoun + " " + num + " " + person + " " + number + " " + poss + " " + case
                    tag_list = tag.split()
                    sum_tag = ' '.join(tag_list)
                    print(sum_tag)
                    word = Word(name = current_word, tag = sum_tag, lemma = lemma)
                    word.save()
                while Word.objects.filter(name = current_word).exists():
                    index += 1
                    if index >= request.session['length']:
    ...
    if request.method == 'POST':
        with open('words.csv', 'wb+') as csv:
            for obj in Word.objects.values():
                line = str(obj.get('id')) + ";" + obj.get('name') + ";"  + obj.get('lemma') +  ";" + obj.get('tag') + "\n"
